...
var arrayDropDowns=[];
//generating 4 dropdowns with associated IDs
var selectCSCS=[];
cscs.forEach(function(X,i){
...
// const COIND=0; STIND=1; CIIND=2; SHIND=3;
// //changing the content of STATE/CITY dropdown based on a hierarchy approach of COUNTRY-> STATE and COUNTRY->CITY
// d3.select('#addcountry').on("change",function(){
//   arrayDropDowns[STIND]=fillDropDown('country','state',COIND,STIND);
//   arrayDropDowns[CIIND]=fillDropDown('country','city',COIND,CIIND)
// });

// //changing the content of CITY/COUNTRY dropdown based on a hierarchy approach of STATE-> CITY and STATE <- COUNTRY
// d3.select('#addstate').on("change",function(){
//   arrayDropDowns[CIIND]=fillDropDown('state','city',STIND,CIIND);
//   arrayDropDowns[COIND]=fillDropDown('state','country',STIND,COIND)
// });

// //changing the content of STATE/COUNTRY dropdown based on a hierarchy approach of CITY<- STATE andd CITY <- COUNTRY
// d3.select('#addcity').on("change",function(){
//   arrayDropDowns[STIND]=fillDropDown('city','state',CIIND,STIND);
//   arrayDropDowns[COIND]=fillDropDown('city','country',CIIND,COIND)
// });
...
function fillDropDown(str1,str2,i,j){
  ...
  if (SelectedIndex){
    //find user selected COUNTRY/STATE based on its index
    Xs=findXandShift(str1)[SelectedIndex];
...
function fillDropParent(str1,X,Xm1,strX){
  ...
  filX=Array.from(new Set(dataX)).map(x => x.toUpperCase());
  if (X === 'city'){
    filX.unshift(`*All ${filX.length} ${strX} of ${Xm1}*`)};
  return filX.sort()
}
